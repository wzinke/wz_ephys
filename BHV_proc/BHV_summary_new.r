...
sesstime = (dt$TrialStart[hitpos] - dt$TrialStart[1])/60000
...
maxY = max(c(relhit$y, rellate$y , RThit$y, relearly$y))
maxX = max(c(relhit$x, RThit$x, relearly$x, rellate$x ))
...
lines(rellate, col='yellow', lwd=2.5)
lines(ChngTime, col='blue', lwd=1.5)

abline(v=dimmitv, lty=3, col='blue')

### RT distribution
max_X = max(c(tail(RThit$x, n=1), tail(RTlate$x, n=1)))
max_Y = max(c(max(RThit$y), max(RTlate$y)))

truehist(dt$RT, h=25, col='darkgrey', main='Reaction Time', xlab='RT [ms]', ylab='count', prob=FALSE, xlim=c(0, max_X), xaxs='i', yaxs='i')
lines(RThit, lwd=2.5, col='red')
lines(RTlate, lwd=2.5, col='yellow')
...
barplot(100 * prop.table(table(resnm[resppos], DimmCnd[resppos]), margin=2), beside=T, space=c(0, 0.4), col=c('red', 'green', 'yellow'), ylab='rate [%]', xlab='dimming interval', main='Performance', yaxs='i')
...
points(sesstime[latepos], reltime[latepos], pch=20, col='yellow')
...
points(dt$ChangeIntent[latepos], reltime[latepos], pch=20, col='yellow')
