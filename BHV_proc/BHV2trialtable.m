...
if(exist('ifl','var') == 0 || isempty(ifl) == 1)
    [ifl, pathname] = uigetfile(['*.bhv'], 'Choose BHV file');
    if ifl == 0,
        return
    end
    ifl = [pathname ifl];
...
for(t=1:numTrials)
    ...
    RewDelay   = max(Get_time(EvTimes,EvCodes, 19));
    RewDelayEv = max(Get_time(EvTimes,EvCodes, 20));
    ...
    Ttbl.RewLag(t,1)    = RewDelayEv - RewDelay;
    Ttbl.RewLagEff(t,1) = RewEv      - RewDelayEv;
