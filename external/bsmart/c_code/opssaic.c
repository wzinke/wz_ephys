...
if (argc==10)
   {
     ...
     NPTS=atoi(argv[7]);
     WIN=atoi(argv[8]);
     MODORDER=atoi(argv[9]);
   }
else if (argc!=5 && argc!=10)
	{
	  fprintf(stderr,"opssaic datfile A Ve AIC Nchannels Ntrails Npoints WIN MODORDER\n");
	  exit(1);
	}
else { 
...
/* convert data format from float to double  */
  ...
  MARfit(x,NCHN,n,NTRLS,MAXTRY-1,tildA);

	 

/* AIC to determine the model order */
  /* jj=0; for(ii=0;ii<NTRLS;ii++) jj += n[ii]; */
  jj = NTRLS*WIN;
  MARgetaic(tildA,NCHN,MAXTRY,jj,aic);
   fp = fopen(argv[4],"a"); 
   
   for(ii=0;ii<MODORDER;ii++)
         /* fprintf(stderr,"%d  %g\n",i,aic[i]);  */
         fprintf(fp,"%g ",aic[ii]);
   fprintf(fp,"\n");
   fclose(fp);
