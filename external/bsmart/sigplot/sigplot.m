function [outvar1] = sigplot(data, varargin)
% SIGPLOT     Scroll (horizontally and/or vertically) through multichannel data.
%             Allows vertical scrolling through channels and manual marking
%             and unmarking of data stretches or epochs for rejection.
% Usage:
%           >> sigplot(data, 'key1', value1 ...); % use interface buttons, etc.
%      else
%           >> sigplot('noui', data, 'key1', value1 ...); % no user interface;
%                                                         % use for plotting
% Menu items:
...
%                  button is pressed (see Outputs); called from pop_sigplot() or
...
%                  Called from command line, subsequent functions sigplot2event() or
%                  sigplot2trials() allow processing trials marked with different colors
...
%                   equivalent is 'title'
%     "Settings > Time range to display"  - [menu] For continuous SIG data, this item
...
%                  pop_sigplot(), the channel labels of the dataset will be used.
...
%                  multitrial 3-D (channels,timepoints,trials). If the data is preceded
...
%                   sigplot2event() and sigplot2trial()).
...
%    'tag'        - [string] Matlab object tag to identify this sigplot() window (allows
%                    keeping track of several simultaneous sigplot() windows).
%    'children'   - [integer] Figure handle of a *dependent* sigplot() window. Scrolling
...
%                    this variable. (See sigplot2trial() and sigplot2event()).
%
% Author: BSMART group, SHIS, U of Texas Health Science Center at Houston,
%         Houston, TX, 2006-2007
%         Arnaud Delorme & Colin Humphries, CNL/Salk Institute, SCCN/INC/UCSD, 1998-2001
%
% See also: eeg_multisigplot(), sigplot2event(), sigplot2trial(), eeglab()
...
% parameter setting up (can be re-defined):
% cannot permute data, because of callback

% DEFAULT_PLOT_COLOR = { [0 0 1], [0.7 0.7 0.7]};         % SIG line color
try
    icadefs;
...
% DEFAULT_NOUI_PLOT_COLOR = 'k';    % SIG line color for noui option
...
% If NOT a 'noui' call or a callback from uicontrols
if ~ischar(data)
    try
        options = varargin;
        ...
        disp('sigplot() error: calling convention {''key'', value, ... } error'); return;
    end%try
    % srate - sampling rate (default = 1 Hz)
    try
        g.srate;
    catch
        g.srate	= 1;
    ...
    try
        ...
        g.eloc_file	= 0;    % 0 mean numbered
    end%try
    % Number of seconds of SIG displayed
    try
        g.winlength;
    catch
        g.winlength	= 5;
    ...
    try
        ...
        g.position = ORIGINAL_POSITION;
    end%try
    % title of the display window
    try
        g.title;
    catch
        g.title	= 'Display of multichannel and multitrial signals';
    end%try
    try
        g.trialstag;
    catch
    ...
    for index=1:length(gfields)
        switch gfields{index}
            ...
            otherwise,
                error(['sigplot: unrecognized option: ''' gfields{index} '''' ]);
        end%switch
    end%for
    ...
    % -------------------------------------------
    tmpcolmodif = zeros(1,length(g.colmodif));
    ...
    if ischar(g.eloc_file) || isstruct(g.eloc_file)  % Read in electrode names
        ...
        sigplot('setelect', g.eloc_file, ax1);
    end;
    ...
    u(1) = uicontrol('Parent',figh, ...
        ...
        'Tag','Pushbutton1',...
        'string','<<',...
        'Callback','sigplot(''drawp'',1)');
    u(2) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position', posbut(2,:), ...
        'Tag','Pushbutton2',...
        'string','<',...
        'Callback','sigplot(''drawp'',2)');
    u(5) = uicontrol('Parent',figh, ...
        ...
        'Callback', 'sigplot(''drawp'',0);' );
    u(3) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position',posbut(3,:), ...
        'Tag','Pushbutton3',...
        'string','>',...
        'Callback','sigplot(''drawp'',3)');
    u(4) = uicontrol('Parent',figh, ...
        'Units', 'normalized', ...
        'Position',posbut(4,:), ...
        'Tag','Pushbutton4',...
        'string','>>',...
        'Callback','sigplot(''drawp'',4)');
    ...
    uimenu('Parent',m(7),'Label','Edit figure', 'Callback', 'sigplot(''noui'');');
    ...
    % Scale Eye %%%%%%%%%
    timestring = ['[OBJ1,FIG1] = gcbo;',...
        'sigplot(''scaleeye'',OBJ1,FIG1);',...
        'clear OBJ1 FIG1;'];
    m(7) = uimenu('Parent',m(1),'Label','Show scale','Callback',timestring);

    % Title %%%%%%%%%%%%
    uimenu('Parent',m(1),'Label','Title','Callback','sigplot(''title'')')
    ...
    uimenu('Parent',m(6),'Label','Load .loc(s) file',...
        'Callback','sigplot(''loadelect'');')

    % Zooms %%%%%%%%
    zm = uimenu('Parent',m(2),'Label','Zoom off/on');
    commandzoom = [ 'set(gcbf, ''windowbuttondownfcn'', [ ''zoom(gcbf,''''down''''); sigplot(''''zoom'''', gcbf, 1);'' ]);' ...
    ...
    uimenu('Parent',zm,'Label','Zoom off', 'separator', 'on', 'callback', ...
        ...
        'clear tmpg;' ]);
    uimenu('Parent',figh,'Label', 'Help', 'callback', 'pophelp(''sigplot'');');

    % Events %%%%%%%%
    zm = uimenu('Parent',m(2),'Label','Events');
    complotevent = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''on'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; sigplot(''drawp'', 0);'];
    comnoevent   = [ 'tmpg = get(gcbf, ''userdata'');' ...
        'tmpg.plotevent = ''off'';' ...
        'set(gcbf, ''userdata'', tmpg); clear tmpg; sigplot(''drawp'', 0);'];
    comeventleg  = 'sigplot(''drawlegend'', gcbf);';
    ...
    commandpush = ['ax1 = findobj(''tag'',''backeeg'',''parent'',gcbf);' ...
        ...
        '     end;' ...
        '     set(gcbf,''UserData'', g);' ...
        '     sigplot(''drawp'', 0);' ...  % redraw background
        '   end;' ...
        'end;' ...
        'clear g hhdat hh tmpelec tmppos ax2 ESpacing lowlim highlim Allwin Fs winlength EPosition ax1 I1 tmpi' ];
    ...
    commandmove = ['ax1 = findobj(''tag'',''backeeg'',''parent'',gcbf);' ...
        ...
        '     sigplotdata = get(ax1, ''userdata'');' ...
        '     set(hh, ''string'', num2str(sigplotdata(g.chans+1-tmpelec, min(g.frames,max(1,double(round(tmppos(1)+lowlim)))))));' ...  % put value in the box
        '  end;' ...
        'end;' ...
        'clear g labls sigplotdata tmpelec nbdat ESpacing tmppos ax1 hh lowlim Fs winlength;' ];
    ...
    % --------------------------
    if ~isempty(g.events)
        if ~isfield(g.events, 'type') || ~isfield(g.events, 'latency') 
            g.events = []; 
        end;
    end;
    ...
    sigplot('scaleeye', [], gcf);
    if strcmpi(g.scale, 'off')
        sigplot('scaleeye', 'off', gcf);
    end;
    ...
    switch data
        case 'drawp' % Redraw SIG and change position
            % this test help to couple sigplot windows
            if exist('p3','var')
                figh = p3;
                figure(p3);
            else
            ...
            if g.children ~= 0
                ...
                sigplot( 'drawp', p1, p2, g.children);
                figure(figh);
            end;
        ...
        case 'drawb' % Draw background ******************************************************
            % Redraw SIG and change position
        ...
        case 'window'  % change window size
            ...
            % -------------------------------------
            g = get(gcf,'UserData');
            result       = inputdlg2( { fastif(g.trialstag==-1,'New window length (s):', 'Number of trial(s):') }, 'Change window length', 1,  { num2str(g.winlength) });
            if size(result,1) == 0 
                return;
            end;
        ...
        case 'winelec'  % change channel window size
            ...
            set(gcf, 'UserData', g);
            sigplot('updateslider', fig);
            sigplot('drawp',0);
            sigplot('scaleeye', [], fig);
            return;
        ...
        case 'loadelect' % load channels
            ...
            sigplot('setelect',[ inputpath inputname ],AXH0);
            return;
        ...
        case 'title'
            ...
            if ~isempty(h)
                ...
                if ~isempty(result), h = textsc(result{1}, 'title'); set(h, 'tag', 'sigplottitle');end;
            end;
        ...
        case 'noui'
            if ~isempty(varargin)
                sigplot( varargin{:} ); fig = gcf;
            else
                fig = findobj('tag', 'SIGPLOT');
            end;
        ...
        case 'zoom' % if zoom
            ...
            if exist('p2') == 1
                zoom on;
                set(gcbf, 'windowbuttondownfcn', 'zoom(gcbf,''down''); sigplot(''zoom'', gcbf, 1);');
        ...
        otherwise
            error(['Error - invalid sigplot() parameter: ',data])
    end
end
