% CROSSCORRELATIONHISTOGRAM calculates cross correlation histogram at a given
	
% crossCorrelationHistogram.m
% 
...
function xcorrHist = crossCorrelationHistogram(jpsth,lag)
	if nargin < 2 lag = 50; end
	
	xcorrHist = zeros(1,length(-lag:lag));
	for i=-lag:lag
		xcorrHist(i+lag+1) = (sum(diag(jpsth,i)))/(length(jpsth)-abs(i));
	end
end
