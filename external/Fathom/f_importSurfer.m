function [ncst,k,Area] = f_importSurfer(fname,utm_zone)
% - import Surfer *.bln blanking file for M_Map
%
% USAGE: [ncst,k,Area] = f_importSurfer('fname','utm_zone');
%
...
% 
% SEE ALSO: f_importShapefile, f_utm2deg, m_subset
...
%
% For converting UTM coordinates to lon/lat this function call's UTM2DEG.M
% which is available from: http://www.mathworks.com/matlabcentral/fileexchange/
% You can use UTMZONE from the Matlab Mapping Toolbox to determine which
% zone to specify.
...
% Jun-2006: add support for converting UTM to LON/LAT
...
% See if UTM ZONE is provided for conversion to LON/LAT:
...
if utm_zone ~= 0
   [lat,lon] = utm2deg(ncst(:,1),ncst(:,2),repmat(utm_zone,size(ncst,1),1));
   ncst = [lon lat];
end
