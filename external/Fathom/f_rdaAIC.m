function [best,cond] = f_rdaAIC(Y,X,BIC,verb,cut,xLabels)
% - stepwise forward selection of explanatory variables in RDA using AIC (or BIC)
%
% USAGE: [best,cond] = f_rdaAIC(Y,X,BIC,verb,cut,xLabels);
%
...
if (nargin < 3), BIC      =   0; end % return AIC by default
if (nargin < 4), verb     =   1; end % send output to display by default
if (nargin < 5), cut      =   2; end % default cut-off value of 2
...
for j = 2:ncX            % 1st variable selected from conditional tests
   ...
   for i = 1:nc_x
      result        = f_rda(Y,[W X(:,i)],[0],0,0,1,0);
...
if (verb>0)
   fprintf('\n==================================================\n');
   if (BIC<1)
      fprintf('AIC-based stepwise forward selection (RDA)         \n');
   else
      fprintf('BIC-based stepwise forward selection (RDA)         \n');
   end
   fprintf('--------------------------------------------------\n');
