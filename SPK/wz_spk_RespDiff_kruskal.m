function Tdiff = wz_spk_RespDiff_kruskal(respmat, condvec, timwin, mincons, pthr, nBoot, smplsz, xvec)
% wz_spk_RespDiff_kruskal - calculate the time when two responses become
% clearly ifferen
%  
% DESCRIPTION 
%
...
if(Ncond < 2)
    warning('Only one condition - nothing to compare!')
    Tdiff = [];
    return
end
...
if(nBoot > 1)
    ...
    for(i=1:Ncond)
        ...
        if(i==1)
            ...
            cs = sum(smplszs(1:i-1))+1; %cumsum(smplszs(i-1));
        end
...
parfor(i=1:length(tpos))  % loop over time bins
    ...
    for(b=1:nBoot) % loop over bootstrap repetitions
        csr  = cBinVals(Bsmpl(b,:)); % resample data
